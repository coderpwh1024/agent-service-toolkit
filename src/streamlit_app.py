...
async def main() -> None:
    ...
    # Bailian voice providers are lightweight and re-read configuration on each rerun.
    ...
    with st.sidebar:
        ...
        st.caption("基于 LangGraph、FastAPI 和 Streamlit 构建的完整 AI 智能体服务工具包")

        if st.button(":material/chat: 新建聊天", type="primary", width="stretch"):
        ...
        with st.expander(":material/history: 历史聊天", expanded=False):
            try:
                ...
                st.caption(f"无法加载聊天记录：{e}")
                thread_list = []

            for t in thread_list:
                label = t.title or f"对话 {t.thread_id[:8]}"
                if st.button(label, key=f"thread_{t.thread_id}", width="stretch"):
                    try:
        ...
        with st.popover(":material/settings: 设置", width="stretch"):
            model_idx = agent_client.info.models.index(agent_client.info.default_model)
            model = st.selectbox(
                "选择大语言模型", options=agent_client.info.models, index=model_idx
            )
            ...
            enable_audio = st.toggle(
                ...
                help="设置 DASHSCOPE_API_KEY 后可启用阿里云百炼语音功能"
            ...
            if voice:
                voice_features = []
                if voice.stt:
                    voice_features.append("语音输入")
                if voice.tts:
                    voice_features.append("语音回复")
                st.caption(f"阿里云百炼：已启用{'、'.join(voice_features)}")

            # Display user ID (for debugging or user information)
            st.text_input("用户标识（只读）", value=user_id, disabled=True)

        st.caption(
            "由 [Joshua](https://www.linkedin.com/in/joshua-k-carroll/) 在奥克兰用 :material/favorite: 打造"
        )
    ...
    # DASHSCOPE_API_KEY automatically enables Bailian voice input and output.
    if voice:

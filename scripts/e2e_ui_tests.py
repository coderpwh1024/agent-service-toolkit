...
def scenario_multi_turn_resume(browser: Browser, base_url: str) -> None:
    """Two-turn conversation, then resume it from the URL in a fresh session.

    Exercises thread persistence and the agent-aware /history fetch on resume.
    """
    ...
    # Rebuild the current conversation URL explicitly. The sidebar intentionally
    # omits a share control, while the URL remains directly resumable/bookmarkable.
    user_id = query_param(page, "user_id")
    if not user_id:
        raise E2EError("user_id was not published to the URL on load")
    ...
    resumed = open_app(browser, resume_url)
    if query_param(resumed, "thread_id") != thread_id:
        raise E2EError("resumed session did not carry the original thread_id from the URL")
...
def scenario_settings_selectors(browser: Browser, base_url: str) -> None:
    ...
    model = selectbox_value(page, "选择大语言模型")
    if not model:
        raise E2EError("LLM selectbox rendered without a selected model")
    log(f"model selectbox shows: {model!r}")

    default_agent = selectbox_value(page, "选择智能体")
    agents = page.locator('[data-testid="stSelectbox"]').filter(has_text="选择智能体")
    ...
    if selectbox_value(page, "选择智能体") != target:
...
def scenario_streaming_toggle(browser: Browser, base_url: str) -> None:
    ...
    page = open_app(browser, base_url, agent=CHAT_AGENT)
    open_settings(page)
    toggle = page.locator('[data-testid="stCheckbox"]').filter(has_text="流式输出")
...
def scenario_new_chat(browser: Browser, base_url: str) -> None:
    ...
    page.get_by_role("button", name="新建聊天").first.click()
...
def scenario_live_model(browser: Browser, base_url: str) -> None:
    ...
    page = open_app(browser, base_url, agent=CHAT_AGENT)
    open_settings(page)
    box = page.locator('[data-testid="stSelectbox"]').filter(has_text="选择大语言模型")

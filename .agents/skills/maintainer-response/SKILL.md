...
**评论的署名完全取决于发布所使用的 GitHub 凭据。** 无论在本地还是 Codex cloud，
都只能通过明确以 Joshua 身份认证的 `gh` CLI 发布（`gh auth login` / PAT）。不要通过
GitHub App、连接器或 MCP 服务器代发，因为它们可能使用不同身份或附加应用标记。
...
5. 先运行 `gh api user --jq .login` 确认身份，结果必须为 `JoshuaC215`。如果不是该用户，
   或写入操作会通过 GitHub App、连接器或 MCP 服务器路由，应停止并告知 Joshua，不得使用错误署名
   发布。
